...
from transformers.utils import ModelOutput, add_start_docstrings, logging, replace_return_docstrings
...
class OmDetTurboEncoderOutput(ModelOutput):
    ...
    Args:
        ...
        attentions (`tuple(mindspore.Tensor)`, *optional*, returned when `output_attentions=True` is passed or when `config.output_attentions=True`):
            ...
            heads.
        extracted_states (`Tuple[mindspore.Tensor]`):
    ...
    extracted_states: Tuple[ms.Tensor] = None
...
class OmDetTurboDecoderOutput(ModelOutput):
    ...
    Args:
        ...
        decoder_coords (`mindspore.Tensor` of shape `(batch_size, num_queries, 4)`):
            The predicted coordinates of the objects.
        decoder_classes (`mindspore.Tensor` of shape `(batch_size, num_queries, num_classes)`):
            The predicted classes of the objects.
        encoder_coord_logits (`mindspore.Tensor` of shape `(batch_size, num_queries, 4)`):
            The predicted coordinates of the objects from the encoder.
        encoder_class_logits (`Tuple[mindspore.Tensor]`) of shape `(batch_size, num_queries, num_classes)`:
        ...
        intermediate_reference_points (`Tuple[Tuple[mindspore.Tensor]]`):
            The intermediate reference points.
        hidden_states (`Optional[Tuple[mindspore.Tensor]]`, *optional*, returned when `output_hidden_states=True` is passed
            or when `config.output_hidden_states=True`):
            Tuple of `mindspore.Tensor` (one for the output of the embeddings + one for the output of each layer) of shape
            `(batch_size, sequence_length, hidden_size)`. Hidden-states of the model at the output of each layer
            plus the initial embedding outputs.
        attentions (`Optional[Tuple[Tuple[mindspore.Tensor]]]`, *optional*, returned when `output_attentions=True` is passed or when
            `config.output_attentions=True`):
            Tuple of tuples of `mindspore.Tensor` (one for attention for each layer) of shape `(batch_size, num_heads,
    ...
    decoder_coords: ms.Tensor = None
    decoder_classes: ms.Tensor = None
    encoder_coord_logits: ms.Tensor = None
    encoder_class_logits: Tuple[ms.Tensor] = None
    init_reference_points: ms.Tensor = None
    intermediate_reference_points: Tuple[Tuple[ms.Tensor]] = None
...
class OmDetTurboObjectDetectionOutput(ModelOutput):
    ...
    Args:
        loss (`mindspore.Tensor`):
            The loss value.
        decoder_coord_logits (`mindspore.Tensor` of shape `(batch_size, num_queries, 4)`):
            The predicted coordinates logits of the objects.
        decoder_class_logits (`mindspore.Tensor` of shape `(batch_size, num_queries, num_classes)`):
        ...
        encoder_coord_logits (`mindspore.Tensor` of shape `(batch_size, num_queries, 4)`):
            The predicted coordinates of the objects from the encoder.
        encoder_class_logits (`Tuple[mindspore.Tensor]`):
            The predicted class of the objects from the encoder.
        encoder_extracted_states (`mindspore.Tensor`):
            The extracted states from the Feature Pyramid Network (FPN) and Path Aggregation Network (PAN) of the encoder.
        decoder_hidden_states (`Tuple[mindspore.Tensor]`, *optional*):
            Tuple of `mindspore.Tensor` (one for the output of the embeddings + one for the output of each layer) of shape
            `(batch_size, sequence_length, hidden_size)`. Hidden-states of the model at the output of each layer
            plus the initial embedding outputs.
        decoder_attentions (`Tuple[Tuple[mindspore.Tensor]]`, *optional*):
            Tuple of tuples of `mindspore.Tensor` (one for attention for each layer) of shape `(batch_size, num_heads,
            sequence_length, sequence_length)`. Attentions weights after the attention softmax, used to compute the
            weighted average in the self-attention, cross-attention and multi-scale deformable attention heads.
        encoder_hidden_states (`Tuple[mindspore.Tensor]`, *optional*):
            ...
            `(batch_size, sequence_length, hidden_size)`. Hidden-states of the model at the output of each layer
            plus the initial embedding outputs.
        encoder_attentions (`Tuple[Tuple[mindspore.Tensor]]`, *optional*):
            Tuple of tuples of `mindspore.Tensor` (one for attention for each layer) of shape `(batch_size, num_heads,
            sequence_length, sequence_length)`. Attentions weights after the attention softmax, used to compute the
            weighted average in the self-attention, cross-attention and multi-scale deformable attention heads.
        classes_structure (`mindspore.Tensor`, *optional*):
            The number of queried classes for each image.
    """

    loss: ms.Tensor = None
    decoder_coord_logits: ms.Tensor = None
    decoder_class_logits: ms.Tensor = None
    init_reference_points: ms.Tensor = None
    intermediate_reference_points: Optional[Tuple[Tuple[ms.Tensor]]] = None
    encoder_coord_logits: ms.Tensor = None
    encoder_class_logits: Tuple[ms.Tensor] = None
    encoder_extracted_states: ms.Tensor = None
    ...
    encoder_attentions: Optional[Tuple[Tuple[ms.Tensor]]] = None
    classes_structure: Optional[ms.Tensor] = None
...
class MultiScaleDeformableAttention(ms.nn.Cell):
    def construct(
        self,
        value: ms.Tensor,
        value_spatial_shapes: ms.Tensor,
        value_spatial_shapes_list: List[Tuple],
        level_start_index: ms.Tensor,
        sampling_locations: ms.Tensor,
        attention_weights: ms.Tensor,
        im2col_step: int,
    ):
...
class OmDetTurboLanguageBackbone(ms.nn.Cell):
    def __init__(self, config: OmDetTurboConfig):
        super().__init__()
        self.model = AutoModel.from_config(config.text_config)
        self.text_projection = ms.Parameter(mint.zeros((config.text_projection_in_dim, config.text_projection_out_dim)))

    def construct(self, hidden_states, mask=None, encode_type="task"):
        ...
        if encode_type == "task":
            ...
            max_pooled_output = pooled_output[mint.arange(pooled_output.shape[0]), hidden_states.argmax(dim=-1)]
...
class OmDetTurboVisionBackbone(ms.nn.Cell):
    def __init__(self, config: OmDetTurboConfig):
        super().__init__()
        ...
        self.layer_norms = ms.nn.CellList(
            [
                mint.nn.LayerNorm(in_channel_dim, eps=config.layer_norm_eps)
...
# Copied from transformers.models.deformable_detr.modeling_deformable_detr.DeformableDetrMultiscaleDeformableAttention with
# DeformableDetr->OmDetTurbo, Deformable DETR->OmDet-Turbo
class OmDetTurboMultiscaleDeformableAttention(ms.nn.Cell):
    """
    ...
    def with_pos_embed(self, tensor: ms.Tensor, position_embeddings: Optional[ms.Tensor]):
...
class OmDetTurboConvNormLayer(ms.nn.Cell):
...
class OmDetTurboRepVggBlock(ms.nn.Cell):
    """
...
class OmDetTurboCSPRepLayer(ms.nn.Cell):
    """
    ...
    def __init__(self, config: OmDetTurboConfig):
        ...
        self.bottlenecks = ms.nn.SequentialCell(*[OmDetTurboRepVggBlock(config) for _ in range(num_blocks)])
...
class OmDetTurboMultiheadAttention(ms.nn.Cell):
    ...
    def __init__(self, config, hidden_size, num_attention_heads, dropout):
        ...
        self.query = mint.nn.Linear(hidden_size, self.all_head_size)
        self.key = mint.nn.Linear(hidden_size, self.all_head_size)
        self.value = mint.nn.Linear(hidden_size, self.all_head_size)
        self.out_proj = mint.nn.Linear(hidden_size, hidden_size)
...
class OmDetTurboEncoderLayer(ms.nn.Cell):
    def __init__(self, config: OmDetTurboConfig):
        super().__init__()
        ...
        self.dropout = mint.nn.Dropout(config.encoder_dropout)
        self.activation_fn = ACT2FN[config.encoder_feedforward_activation]
        self.encoder_feedforward_dropout = mint.nn.Dropout(config.encoder_feedforward_dropout)
        self.fc1 = mint.nn.Linear(config.encoder_hidden_dim, config.encoder_dim_feedforward)
        self.fc2 = mint.nn.Linear(config.encoder_dim_feedforward, config.encoder_hidden_dim)
...
class OmDetTurboEncoder(ms.nn.Cell):
    def __init__(self, config: OmDetTurboConfig):
        super().__init__()

        self.layers = ms.nn.CellList([OmDetTurboEncoderLayer(config) for _ in range(config.encoder_layers)])
...
class OmDetTurboHybridEncoder(ms.nn.Cell):
    """
    ...
    def __init__(self, config: OmDetTurboConfig):
        ...
        self.channel_projection_layers = ms.nn.CellList()
        for in_channel in self.in_channels:
            self.channel_projection_layers.append(
                ms.nn.SequentialCell(
                    mint.nn.Conv2d(in_channel, self.encoder_hidden_dim, kernel_size=(1, 1), bias=False),
                    mint.nn.BatchNorm2d(self.encoder_hidden_dim),
                )
            )

        # encoder transformer
        self.encoder = ms.nn.CellList([OmDetTurboEncoder(config) for _ in range(len(self.encoder_projection_indices))])
        # top-down fpn
    ...
    def construct(
        ...
        r"""
        Args:
            inputs_embeddings (`mindspore.Tensor` of shape `(batch_size, sequence_length, hidden_size)`):
        ...
        for idx in range(len(self.in_channels) - 1, 0, -1):
            ...
            upsample_feat = mint.nn.functional.interpolate(feat_high, scale_factor=2.0, mode="nearest")
            fps_map = self.fpn_blocks[len(self.in_channels) - 1 - idx](mint.concat([upsample_feat, feat_low], dim=1))
        ...
        for idx in range(len(self.in_channels) - 1):
            ...
            hidden_states = self.pan_blocks[idx](mint.concat([downsample_feat, feat_high], dim=1))
...
class OmDetTurboMLPWithDropout(ms.nn.Cell):
    def __init__(self, config):
        super().__init__()
        self.linear1 = mint.nn.Linear(config.class_embed_dim, config.task_encoder_hidden_dim)
        self.activation = ACT2FN[config.decoder_activation]
        self.dropout = mint.nn.Dropout(config.decoder_dropout)
        self.linear2 = mint.nn.Linear(config.task_encoder_hidden_dim, config.class_embed_dim)

    def construct(self, x):
        return self.linear2(self.dropout(self.activation(self.linear1(x))))


class OmDetTurboMLP(ms.nn.Cell):
    ...
    def __init__(self, input_dim, hidden_dim, output_dim, num_layers):
        ...
        self.layers = ms.nn.CellList(
            [mint.nn.Linear(in_dim, out_dim) for in_dim, out_dim in zip(layers_dims[:-1], layers_dims[1:])]
        )
...
class OmDetTurboResidualLayer(ms.nn.Cell):
    """
    ...
    def __init__(self, config):
        super().__init__()
        self.norm1 = mint.nn.LayerNorm(config.class_embed_dim, eps=config.layer_norm_eps)
        self.dropout = mint.nn.Dropout(config.decoder_dropout)
...
class OmDetTurboTaskEncoder(ms.nn.Cell):
    def __init__(self, config):
        super().__init__()
...
class OmDetTurboDeformableTransformerDecoderLayer(ms.nn.Cell):
    """
    ...
    def __init__(self, config):
        ...
        self.dropout1 = mint.nn.Dropout(config.decoder_dropout)
        self.norm1 = mint.nn.LayerNorm(config.decoder_hidden_dim, eps=config.layer_norm_eps)
        ...
        self.dropout2 = mint.nn.Dropout(config.decoder_dropout)
        self.norm2 = mint.nn.LayerNorm(config.decoder_hidden_dim, eps=config.layer_norm_eps)

        # feed forward network
        self.linear1 = mint.nn.Linear(config.decoder_hidden_dim, config.decoder_dim_feedforward)
        self.act = ACT2FN[config.decoder_activation]
        self.dropout3 = mint.nn.Dropout(config.decoder_dropout)
        self.linear2 = mint.nn.Linear(config.decoder_dim_feedforward, config.decoder_hidden_dim)
        self.dropout4 = mint.nn.Dropout(config.decoder_dropout)
        self.norm3 = mint.nn.LayerNorm(config.decoder_hidden_dim, eps=config.layer_norm_eps)
    ...
    def construct(
        ...
        query = mint.cat((query, task_features), dim=1)
        key = mint.cat((key, task_features), dim=1)
        decoder_embeddings = mint.cat((decoder_embeddings, task_features), dim=1)
...
class OmDetTurboPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        # MindSpore: replace torch.nn.init.uniform_ with initializer(Uniform(...))
        def linear_init_(module_to_init):
            bound = 1 / math.sqrt(module_to_init.weight.shape[0])
            module_to_init.weight.set_data(
                initializer(Uniform(bound), module_to_init.weight.shape, module_to_init.weight.dtype)
            )
            if hasattr(module_to_init, "bias") and module_to_init.bias is not None:
                module_to_init.bias.set_data(
                    initializer(Uniform(bound), module_to_init.bias.shape, module_to_init.bias.dtype)
                )
        ...
        if isinstance(module, OmDetTurboEncoderLayer):
            ...
            constant_(module.encoder_bbox_head.layers[-1].weight, 0.0)
            constant_(module.encoder_bbox_head.layers[-1].bias, 0.0)

            for mlp in module.decoder_bbox_head:
                constant_(mlp.layers[-1].weight, 0.0)
                constant_(mlp.layers[-1].bias, 0.0)

            linear_init_(module.encoder_vision_features[0])
            module.encoder_vision_features[0].weight.set_data(
                initializer(
                    XavierUniform(),
                    module.encoder_vision_features[0].weight.shape,
                    module.encoder_vision_features[0].weight.dtype,
                )
            )

            if module.learn_initial_query:
                module.tgt_embed.weight.set_data(
                    initializer(XavierUniform(), module.tgt_embed.weight.shape, module.tgt_embed.weight.dtype)
                )

            module.query_position_head.layers[0].weight.set_data(
                initializer(
                    XavierUniform(),
                    module.query_position_head.layers[0].weight.shape,
                    module.query_position_head.layers[0].weight.dtype,
                )
            )
            module.query_position_head.layers[1].weight.set_data(
                initializer(
                    XavierUniform(),
                    module.query_position_head.layers[1].weight.shape,
                    module.query_position_head.layers[1].weight.dtype,
                )
            )

            for layer in module.channel_projection_layers:
                layer[0].weight.set_data(initializer(XavierUniform(), layer[0].weight.shape, layer[0].weight.dtype))
    ...
    def get_cached_class_embeddings(self, classes_input_ids, classes_attention_mask):
        ...
        if not_cached_classes:
            not_cached_classes_ids = mint.stack([classes_input_ids[idx] for idx in not_cached_index])
        ...
        total_class_embs = mint.stack(total_embeddings)
    ...
    def get_cached_task_embeddings(self, tasks_input_ids, tasks_attention_mask):
        ...
        if not_cached_tasks:
            not_cached_index_ids = mint.stack([tasks_input_ids[idx] for idx in not_cached_index])
            not_cached_mask = mint.stack([tasks_attention_mask[idx] for idx in not_cached_index])
            ...
            for idx in range(embeddings.shape[1]):
                emb = embeddings[:, [idx], :]
                idx_to_put = not_cached_index[idx]
                cur_mask = mint.unsqueeze(masks[idx], dim=0)
        ...
        for idx, task in enumerate(total_task_features):
            if task.shape[0] < max_len:
                ...
                total_task_features[idx] = mint.nn.functional.pad(task, (0, 0, 0, 0, 0, pad_size))
                total_task_masks[idx] = mint.nn.functional.pad(total_task_masks[idx], (0, pad_size))

        total_task_features = mint.cat(total_task_features, dim=1)
        total_task_masks = mint.cat(total_task_masks, dim=0)
    ...
    def get_language_embedding(
        ...
        max_class_size = mint.max(classes_structure)
        class_embeddings_regrouped = []
        start = 0
        for size in classes_structure:
            pad_size = max_class_size - size
            class_embeddings_regrouped.append(
                mint.nn.functional.pad(batched_classes_embeddings[start : start + size], (0, 0, 0, pad_size)).unsqueeze(
                    1
                )
            )
            start += size
        class_embeddings = mint.cat(class_embeddings_regrouped, dim=1)
...
OMDET_TURBO_INPUTS_DOCSTRING = r"""
    Args:
        ...
        classes_input_ids (`mindspore.Tensor` of shape `(total_classes (>= batch_size), sequence_length)`):
        ...
        classes_attention_mask (`mindspore.Tensor` of shape `(total_classes (>= batch_size), num_classes, sequence_length)`):
            Attention mask for the classes. This is a binary mask that indicates which tokens should be attended to,
            and which should not.

        tasks_input_ids (`mindspore.Tensor` of shape `(batch_size, sequence_length)`):
        ...
        tasks_attention_mask (`mindspore.Tensor` of shape `(batch_size, sequence_length)`):
            Attention mask for the tasks. This is a binary mask that indicates which tokens should be attended to,
            and which should not.

        classes_structure (mindspore.Tensor of shape `(batch_size)`):
...
def _cosine_similarity_scaled(a, b, logit_scale):
    ...
    logits_per_image = logit_scale * mint.bmm(a, b)
    return logits_per_image


def get_class_similarity(class_distance_type, cls_feature, class_proj):
    logit_scale = ms.Tensor(1 / 0.07).log()
    if class_distance_type == "cosine":
        ...
        class_logits = mint.bmm(cls_feature, class_proj)
    else:
...
class OmDetTurboDecoder(OmDetTurboPreTrainedModel):
    def __init__(self, config: OmDetTurboConfig):
        ...
        self.channel_projection_layers = ms.nn.CellList(
            ms.nn.SequentialCell(mint.nn.Conv2d(x, hidden_dim, 1, bias=False), mint.nn.BatchNorm2d(hidden_dim))
        ...
        if config.class_embed_dim != hidden_dim:
            self.task_project = mint.nn.Linear(config.class_embed_dim, hidden_dim)
        ...
        if self.learn_initial_query:
            self.tgt_embed = mint.nn.Embedding(self.num_queries, hidden_dim)
        self.query_position_head = OmDetTurboMLP(
            input_dim=4, hidden_dim=2 * hidden_dim, output_dim=hidden_dim, num_layers=2
        )

        # encoder head
        self.encoder_vision_features = ms.nn.SequentialCell(
            mint.nn.Linear(hidden_dim, hidden_dim), mint.nn.LayerNorm(hidden_dim, eps=config.layer_norm_eps)
        )
        self.encoder_class_head = mint.nn.Linear(config.class_embed_dim, hidden_dim)
        self.encoder_bbox_head = OmDetTurboMLP(input_dim=hidden_dim, hidden_dim=hidden_dim, output_dim=4, num_layers=3)

        # decoder head
        self.decoder_class_head = ms.nn.CellList(
            [mint.nn.Linear(config.class_embed_dim, hidden_dim) for _ in range(config.decoder_num_layers)]
        )
        self.decoder_bbox_head = ms.nn.CellList(
    ...
    def generate_anchors(self, spatial_shapes=None, grid_size=0.05, dtype=ms.float32):
        ...
        anchors = mint.where(valid_mask, anchors, float("inf"))
    ...
    def _get_encoder_input(self, vision_features):
        ...
        new_vision_features = mint.cat(new_vision_features, 1)
        new_vision_shapes = ms.Tensor(new_vision_shapes_list, dtype=ms.int64)
        level_start_index = mint.cat((new_vision_shapes.new_zeros((1,)), new_vision_shapes.prod(1).cumsum(0)[:-1]))
    ...
    def _get_decoder_input(
        ...
        anchors, valid_mask = self.generate_anchors(vision_shapes, dtype=vision_features.dtype)
        predicted_class_features = self.encoder_vision_features(
            mint.where(valid_mask, vision_features, ms.Tensor(0.0, dtype=vision_features.dtype))
        )
        ...
        topk_ind = mint.topk(encoder_class_similarity.max(-1).values, self.num_queries, dim=1).indices.view(-1)
        # (batch_size, num_queries)
        batch_ind = (
            mint.arange(
                end=batch_size,
                dtype=topk_ind.dtype,
            )
            .unsqueeze(-1)
        ...
        if denoise_bboxes is not None:
            reference_points = mint.cat([denoise_bboxes, reference_points], 1)
        if self.training:
        ...
        if denoise_embeddings is not None:
            embeddings = mint.cat([denoise_embeddings, embeddings], 1)
    ...
    def construct(
        ...
        """
        Args:
            vision_features (`mindspore.Tensor`): The sequence of vision features. shape depends on the vision
                backbone.
            class_features (`mindspore.Tensor`): The sequence of class features of shape
                `(class_sequence_length, batch_size, class_embed_dim)`.
            task_features (`mindspore.Tensor`): The sequence of task features of shape
                `(task_sequence_length, batch_size, decoder_hidden_dim)`.
            task_mask (`mindspore.Tensor`): The mask for the task features of shape `(batch_size, task_sequence_length)`.
        ...
        key_padding_mask = mint.zeros((batch_size, fusion_size), dtype=ms.bool_)
        ...
        decoder_bboxes = mint.stack(decoder_bboxes)
        decoder_classes = mint.stack(decoder_classes)
...
class OmDetTurboForObjectDetection(OmDetTurboPreTrainedModel):
    ...
    def construct(
        ...
        classes_input_ids: ms.Tensor,
        classes_attention_mask: ms.Tensor,
        tasks_input_ids: ms.Tensor,
        tasks_attention_mask: ms.Tensor,
        classes_structure: ms.Tensor,
        labels: Optional[ms.Tensor] = None,
        output_attentions: Optional[bool] = None,
        output_hidden_states: Optional[bool] = None,
        return_dict: Optional[bool] = None,
    ) -> Union[Tuple[ms.Tensor], OmDetTurboObjectDetectionOutput]:
        r"""
        Returns:
